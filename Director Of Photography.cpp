...
Define a lambda function called bounded, which takes an integer x and returns the integer max(0, min(x, N)).
Iterate through the string C from left to right, and for each character:
a. If it is 'A', do the following:
i. Calculate the left window start and end indices as i-Y and i-X+1 respectively, and apply the bounded function to both.
ii. Calculate the right window start and end indices as i+X and i+Y+1 respectively, and apply the bounded function to both.
iii. Calculate the leftPs as prefixPSum[leftWindowEnd] - prefixPSum[leftWindowStart], and rightBs as prefixBSum[rightWindowEnd] - prefixBSum[rightWindowStart].
iv. Add the product of leftPs and rightBs to the result.
v. Calculate the rightPs as prefixPSum[rightWindowEnd] - prefixPSum[rightWindowStart], and leftBs as prefixBSum[leftWindowEnd] - prefixBSum[leftWindowStart].
